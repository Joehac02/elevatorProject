...
#pragma config(Sensor, dgtl9,  light2,         sensorLEDtoVCC)
	...
	while(true)
	{
		while(floorNumber == 1)
		{
			...
			if(SensorValue(floorThreeButton) == 1)
			{
				...
				turnLEDOn(light3);
				startMotor(driver, -63);
				untilEncoderCounts(-1400,encoder);
				stopMotor(driver);
				floorNumber = 3;
				clearTimer(T1);
			}
		...
		while(floorNumber == 2) //while on floor two
		{
			if(SensorValue(floorTwoButton)== 1) //floor two gets pressed
			{
				clearTimer(T1);
				turnLEDOn(light2);
				wait(1);
			...
			if(SensorValue(floorThreeButton) == 1)
			{
				...
				turnLEDOn(light3);
				startMotor(driver, -63);
				untilEncoderCounts(-520,encoder);
				stopMotor(driver);
				floorNumber = 3;
				clearTimer(T1);
			}
		...
		while(floorNumber == 3)
		{
			...
			if(SensorValue(floorTwoButton) == 1)
			{
				...
				turnLEDOn(light2);
				startMotor(driver, 63);
				untilEncoderCounts(480,encoder);
				stopMotor(driver);
				floorNumber = 2;
				clearTimer(T1);
			}
...
{
	startMotor (driver, 63);
	untilEncoderCounts(1360, encoder);
	stopMotor (driver);
	turnLEDOn(light1);
	floorNumber = 1;
	clearTimer(T1);
